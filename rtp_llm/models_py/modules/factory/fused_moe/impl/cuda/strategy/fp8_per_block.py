...
class CudaSm120Fp8GroupedGemmNoDPStrategy(MoeStrategy):
    """SM12x FP8 PerBlock PureTP strategy using Triton grouped GEMM.

    This is the consumer-Blackwell alternative to the DeepGEMM path, whose
    kernels do not support SM12x. The executor currently allocates dynamic
    scratch buffers, so CUDA graph capture is deliberately rejected.
    """
    ...
    def check_conditions(cls, checker: Any, config: MoEConfigAdapter) -> None:
        ...
        checker.check(config.moe_strategy in ("auto", "fp8_per_block_sm120_grouped"))
