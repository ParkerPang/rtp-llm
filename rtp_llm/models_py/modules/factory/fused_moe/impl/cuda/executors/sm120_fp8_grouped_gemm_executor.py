...
class Sm120Fp8GroupedGemmExecutor(FusedMoeExpertExecutor):
    ...
    def __init__(
        ...
        if self.K % self.BLOCK_SIZE != 0:
            raise ValueError(
                f"SM120 grouped MoE hidden size K={self.K} must be divisible by 128"
            )
        if self.N % (2 * self.BLOCK_SIZE) != 0:
            raise ValueError(
                f"SM120 grouped MoE gate/up size N={self.N} must be divisible by 256"
            )
        if tuple(self.w2_weight.shape) != (self.E, self.K, self.N // 2):
            raise ValueError(
                "SM120 grouped MoE w2 shape must be "
                f"({self.E}, {self.K}, {self.N // 2}), got {tuple(self.w2_weight.shape)}"
            )
        expected_w13_scale = (
            self.E,
        ...
        expected_w2_scale = (
            self.E,
            self.K // self.BLOCK_SIZE,
            (self.N // 2) // self.BLOCK_SIZE,
        )
        if tuple(self.w13_scale.shape) != expected_w13_scale:
            raise ValueError(
                f"SM120 grouped MoE w13 scale shape must be {expected_w13_scale}, "
                f"got {tuple(self.w13_scale.shape)}"
            )
        if tuple(self.w2_scale.shape) != expected_w2_scale:
            raise ValueError(
                f"SM120 grouped MoE w2 scale shape must be {expected_w2_scale}, "
                f"got {tuple(self.w2_scale.shape)}"
            )
    ...
    def execute(
        ...
        if activation.lower() not in ("silu", "swiglu", "siglu"):
            raise ValueError(
                f"SM120 grouped MoE supports only SiLU/SiGLU/SwiGLU, got {activation!r}"
            )
        if expert_map is not None:
            raise ValueError("SM120 grouped MoE does not support expert_map")
        if apply_router_weight_on_input:
            raise ValueError(
                "SM120 grouped MoE applies router weights during gather, not on input"
            )
