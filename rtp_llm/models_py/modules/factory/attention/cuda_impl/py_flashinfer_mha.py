...
class PyFlashinferPagedPrefillImpl(FMHAImplBase):
    ...
    def prepare_cuda_graph(self, attn_inputs: PyAttentionInputs) -> None:
        ...
        # The CUDA graph runner updates capture-time input buffers in place;
        # position_ids and sequence metadata therefore keep stable addresses.
        # kv_cache_offset is the only rope parameter whose backing tensor may
        # change and must be copied explicitly for replay.
        new_rope_params = self.rope_kvcache_impl.prepare(attn_inputs)
        for field in ("max_seq_len", "max_prefix_length"):
            captured = getattr(self.rope_params, field)
            replay = getattr(new_rope_params, field)
            if replay > captured:
                raise RuntimeError(
                    "Paged prefill CUDA graph replay exceeds captured "
                    f"{field}: capture={captured}, replay={replay}"
                )
        for field in (
            "padding_offset",
            "position_ids",
            ...
            captured = getattr(self.rope_params, field)
            replay = getattr(new_rope_params, field)
            if (captured is None) != (replay is None) or (
                captured is not None and captured.data_ptr() != replay.data_ptr()
            ):
                raise RuntimeError(
                    "Paged prefill CUDA graph replay changed backing storage "
                    f"for {field}"
                )

        new_kv_cache_offset = new_rope_params.kv_cache_offset
        captured_has_offset = self.rope_params.kv_cache_offset is not None
        replay_has_offset = new_kv_cache_offset is not None
        if captured_has_offset != replay_has_offset:
            raise RuntimeError(
                "Paged prefill CUDA graph replay changed kv_cache_offset "
                f"availability: capture={captured_has_offset}, "
                f"replay={replay_has_offset}"
            )
    ...
    def support(attn_configs: AttentionConfigs, attn_inputs: PyAttentionInputs) -> bool:
        ...
        RopeStyle.No remains valid when KV cache writes are needed: the fused
        writer skips RoPE and still stores K/V. This contract is covered by
        test_trtllm_fmha_v2_paged_prefill.test_impl_without_rope_matches_torch.
        """
        return (
            not is_sm10x()
...
class PyFlashinferPrefillImpl(FMHAImplBase):
    """FlashInfer ragged prefill with optional fused RoPE/KV-cache writes.

    Ragged prefill is also the encoder-only fallback when
    ``need_rope_kv_cache`` is false. Runtime behavior follows the actual RoPE
    style and KV-cache presence: the fused writer is skipped only when both
    RoPE is disabled and no KV cache is supplied.
    """
    ...
    def support(attn_configs: AttentionConfigs, attn_inputs: PyAttentionInputs) -> bool:
        ...
        return (
            ...
            and (
                attn_configs.need_rope_kv_cache
                or attn_configs.rope_config.style == RopeStyle.No
            )
            and attn_configs.rope_config.style != RopeStyle.Mrope
        )
...
class PyFlashinferDecodeAttnOp(object):
    ...
    def prepare_for_cuda_graph_replay(self, attn_inputs: PyAttentionInputs) -> None:
        ...
        # plan_info is derived from runtime page tables/kv lengths. CUDA graph
        # replay keeps wrapper buffer addresses fixed, but their contents and
        # derived plan metadata must be refreshed before every replay.
        self._plan_decode_wrapper(attn_inputs)
