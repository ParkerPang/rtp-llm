    ...
    for (auto& stream : stream_groups.allStreams()) {
        ...
        auto task = [this,
            ...
            dispatchSingleStream(
                stream, merge_outputs, batch_idx_in, batch_idx_out, token_offset, return_all_probs, new_tokens_all, token_ids_cpu, success_cpu);
        };
...
void NormalOutputDispatcher::dispatchSingleStream(GenerateStreamPtr    stream,
    ...
    for (size_t i = 0; i < next_batch_size; ++i) {
        ...
        if (has_beam_search) {
            ...
            token_id       = new_all_token_ids.data_ptr<int32_t>()[(batch_idx_out + i) * token_stride + seq_pos];
        } else {
            token_id = new_all_token_ids.data_ptr<int32_t>()[(batch_idx_out + i) * token_stride + token_stride - 1];
        }
        new_tokens.data_ptr<int32_t>()[i] = token_id;
    }
