...
ParamsPtr FlashInferAttnParams::prepare(const rtp_llm::AttentionConfigs& attn_configs,
    ...
    if (is_prefill) {
        ...
        for (int i = 0; i < batch_size; i++) {
            page_num += input_lengths_host.data_ptr<int>()[i] / tokens_per_block + 1;
        }
    } else {
        input_token_num = input_lengths_host.size(0);
        for (int i = 0; i < batch_size; i++) {
            page_num += sequence_lengths_host.data_ptr<int>()[i] / tokens_per_block + 1;
        }
    }

    auto params = FlashInferAttnParams::create(
        max(MIN_CACHE_BATCH_SIZE, batch_size), max(MIN_CACHE_INPUT_TOKEN_NUM, input_token_num), max(MIN_CACHE_PAGE_NUM, page_num));
