...
class BackendRPCServerVisitorTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_master_route_uses_safe_prefix_cache_keys_with_input_embeddings(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.seq_size_per_block = 2
        visitor.master_client = Mock()
        visitor.master_client.get_backend_role_addrs = AsyncMock(
    ...
    async def test_master_route_skips_cache_keys_when_embedding_starts_at_zero(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.seq_size_per_block = 2
        visitor.master_client = Mock()
        visitor.master_client.get_backend_role_addrs = AsyncMock(
    ...
    async def test_enqueue_keeps_reuse_with_input_embeddings_after_prefix(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
    ...
    async def test_enqueue_disables_reuse_when_input_embedding_starts_at_zero(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 16
        visitor.sp_config = None
        visitor.host_service = Mock(service_available=False)
    ...
    async def test_batch_enqueue_keeps_reuse_with_input_embeddings_after_prefix(self):
        visitor = BackendRPCServerVisitor.__new__(BackendRPCServerVisitor)
        visitor.max_seq_len = 16
        visitor.sp_config = None
        visitor.host_service = Mock(service_available=False)
    ...
    def test_first_input_embedding_loc_uses_min_loc(self):
        input = make_generate_input(
            InputEmbeddings(
                embeddings=[
        ...
        self.assertEqual(first_input_embedding_loc(input), 1)
